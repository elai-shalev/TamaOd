...
def get_all_places_in_radius(coordinate, radius):
  ...
  #params["geometry"] = f'{{"x": {coordinate[0]} ,"y": {coordinate[1]} , "spatialReference": {{"wkid": 4326}}}}'
  #params['geometry'] = quote(params['geometry'])
  params["distance"] = f'{radius}'
...
# Define headers
  ...
  try:
    # TODO - The paramaters aren't getting created correctly, so the request returns empty / all the db is returned
    print(url)
    print(params)
